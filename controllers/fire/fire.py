...
class Fire(Supervisor):
    FLAME_CYCLE = 13     # 13 animation frames per fire cycle
    FLAME_PEAK = 17      # fire starts shrinking after FLAME_PEAK × FLAME_CYCLE updates
    MAX_PROPAGATION = 5  # max spread distance in metres
    MAX_EXTINCTION = 8   # max distance from tree where water extinguishes fire (metres)
    FIRE_DURATION = 30   # simulation steps between fire animation updates (30 × 32 ms ≈ 1 s)
    ...
    def __init__(self):
        ...
        if n == 0:
            ...
            # Pre-select fires in the patrol area; ignited once drone reaches cruise altitude.
            patrol_trees = [t for t in self.trees
                            if 11 <= t.translation[0] <= 21 and 11 <= t.translation[1] <= 21]
            seed_pool = patrol_trees if patrol_trees else self.trees
            self._pending_fires = random.sample(seed_pool, min(3, len(seed_pool)))
            print("Fire plan: {} trees selected in the patrol zone (ignition pending).".format(
        ...
        if self.update_fire:
            ...
            if tree.fire_count % self.FLAME_CYCLE == 0:
                ...
                if tree.fire_scale < tree.scale:
                    tree.stopFire()
            # Animation: stride is always 100000 (independent of scale) so every frame
            # renders at the tree's original Y position regardless of fire size.
            t = [tree.fire_translation[0], tree.fire_translation[1], tree.fire_translation[2]]
            t[1] -= 100000 * (tree.fire_count % 13)
    ...
    def broadcastFires(self):
        if self.emitter is None:
            return
        burning = [list(t.translation) for t in self.trees if t.fire is not None]
        self.emitter.send(json.dumps({"fires": burning}).encode())

    def checkExtinction(self, tree):
    ...
    def run(self):
        ...
        while True:
            ...
            self._broadcast_clock += 1
            if self._broadcast_clock % 10 == 0:
                self.broadcastFires()
            ...
            if start_fire_now:
                ...
                # Ignite fires the moment the first drone reaches cruise altitude (~30 m).
                # This is AFTER the climb completes so fire appears as the drones level off.
                for robot in self.robots:
                    if robot.name == "Mavic 2 PRO" and robot.altitude() > 30:
                        start_fire_now = True
                        for tree in self._pending_fires:
                            self.ignite(tree)
                        print("Drone at {:.1f} m — {} fires ignited in the patrol zone.".format(
                            robot.altitude(), len(self._pending_fires)))
                        break
